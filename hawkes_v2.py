...
class JointRec(nn.Module):
	...
	def __init__(self, num_users:int, num_items:int, embedding_k:int, device, depth:int=0):
		...
		self.item_embedding = nn.Embedding(num_items, embedding_k)
		...
		base_fn.append(nn.Softplus())
		self.base_fn = nn.Sequential(*base_fn).to(args.device)

		amplitude_fn = [nn.Linear(embedding_k, embedding_k//2), nn.Tanh()]
		for _ in range(depth):
			amplitude_fn.append(nn.Linear(embedding_k//2, embedding_k//2))
			amplitude_fn.append(nn.Tanh())
		amplitude_fn.append(nn.Linear(embedding_k//2, 1, bias=False))
		amplitude_fn.append(nn.Tanh())
		self.amplitude_fn = nn.Sequential(*amplitude_fn).to(args.device)

	def forward(self, batch_items, batch_users, batch_time, batch_time_all):
		user_embed = self.user_embedding(batch_users)
	...
	def hawkes_process(self, batch_items, batch_time, batch_time_all):
		item_embed = self.item_embedding(batch_items)
		...
		"""compute"""
		base = base_fn(item_embed)
		amplitude = amplitude_fn(item_embed)
		intensity_all = base + (amplitude * torch.exp(-self.soft(self.intensity_decay) * batch_time_delta * batch_time_mask) * batch_time_mask).cumsum(-1)
		intensity_all = torch.concat([base, intensity_all[:,:-1]], -1)
		"""numerator"""
		intensity = ((nn.ReLU()(intensity_all) + 1e-9).log() * (batch_time_all <= batch_time)).sum(-1)
		"""denorminator"""
		intensities_neg = intensity_all < 0 
		restart_time = batch_time_all + ((torch.where(intensities_neg, base - intensity_all, base) / (base)).log()) / self.soft(self.intensity_decay)
		...
		valid_compensator = ((batch_time_all > restart_time) & (batch_time_all <= batch_time)).int()
		integral_term1 = (base * (batch_time_all - restart_time) * valid_compensator).sum(-1)
		integral_term2 = ((intensity_all - base) / self.soft(self.intensity_decay) * valid_compensator).sum(-1)
		integral_exp_term1 = torch.exp(-self.soft(self.intensity_decay) * (restart_time - dump_time))
		integral_exp_term2 = torch.exp(-self.soft(self.intensity_decay) * (batch_time_all - dump_time))
		integral_exp_term = ((integral_exp_term1 - integral_exp_term2) * valid_compensator).sum(-1)
...
#     def __init__(self, num_users:int, num_items:int, num_stages:int, embedding_k:int, all_popularity:int, device, depth:int=0):
        # super(JointRec, .__init__()
num_users = dataset.n_user
num_items = dataset.m_item
embedding_k = 64
depth = 0

user_embedding = nn.Embedding(num_users, embedding_k)
item_embedding = nn.Embedding(num_items, embedding_k).to(args.device)

intensity_decay = torch.autograd.Variable(torch.randn(1), requires_grad=True).to(args.device) # bet
soft = nn.Softplus()
...
base_fn.append(nn.Linear(embedding_k//2, 1, bias=False))
base_fn.append(nn.Softplus())
base_fn = nn.Sequential(*base_fn).to(args.device)
...
amplitude_fn.append(nn.Linear(embedding_k//2, 1, bias=False))
amplitude_fn.append(nn.Tanh())
amplitude_fn = nn.Sequential(*amplitude_fn).to(args.device)
...
item_embed = item_embedding(batch_items.to(args.device))
base = base_fn(item_embed)
amplitude = amplitude_fn(item_embed)


intensity_all = (base + (amplitude * torch.exp(-soft(intensity_decay) * batch_time_delta * batch_time_mask) * batch_time_mask).cumsum(-1))[:,:-1]
intensity_all = torch.concat([base, intensity_all], -1)
...
"""denorminator"""
intensities_neg = intensity_all < 0 
restart_time = batch_time_all + ((torch.where(intensities_neg, base - intensity_all, base) / (base)).log()) / soft(intensity_decay)
restart_time = torch.concat([torch.zeros_like(restart_time[:,0:1]), restart_time[:, :-1]], -1)
dump_time = torch.concat([torch.zeros_like(restart_time[:,0:1]), batch_time_all[:, :-1]], -1)

restart_time[:,0].sum()
batch_time_all[:,0].sum()
(base - intensity_all[:,0:1]).sum()

valid_compensator = ((batch_time_all > restart_time) & (batch_time_all <= batch_time)).int()
# valid_compensator = ((batch_time_all > restart_time) & batch_time_target).int()
integral_term1 = (base * (batch_time_all - restart_time) * valid_compensator).sum(-1)

integral_term2 = ((intensity_all - base) / soft(intensity_decay) * valid_compensator).sum(-1)
integral_exp_term1 = torch.exp(-soft(intensity_decay) * (restart_time - dump_time))
integral_exp_term2 = torch.exp(-soft(intensity_decay) * (batch_time_all - dump_time))
integral_exp_term = ((integral_exp_term1 - integral_exp_term2) * valid_compensator).sum(-1)

compensator = integral_term1 + integral_term2 * integral_exp_term
(-intensity + compensator).mean()

#%%
intensity = (nn.ReLU()(intensity_all * batch_time_mask))
...
integral_term2 = (intensity_all - base) / soft(intensity_decay) * valid_compensator
integral_exp_term1 = torch.exp(-soft(intensity_decay) * (restart_time - dump_time))
integral_exp_term2 = torch.exp(-soft(intensity_decay) * (batch_time_all - dump_time))
integral_exp_term = ((integral_exp_term1 - integral_exp_term2) * valid_compensator)
compensator = integral_term1 + integral_term2 * integral_exp_term

compensator = torch.concat([batch_time_all[:, 0:1] * base, compensator[:, 1:]], -1)
batch_time_mask
hawkes_density = (intensity+1e-9) / (compensator+1e-9)
batch_time_all
...
#%%
dataset.item_time_array
total_batch = dataset.m_item // args.batch_size
