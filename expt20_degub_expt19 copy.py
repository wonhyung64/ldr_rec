...
class JointRec(nn.Module):
	...
	def __init__(self, num_users:int, num_items:int, embedding_k:int, mini_batch, device, depth:int=0, tau:float=0.5):
		...
		self.user_bias = nn.Sequential(*[nn.Linear(embedding_k, 1),])
		self.item_bias = nn.Sequential(*[nn.Linear(embedding_k, 1)])
	...
	def interaction(self, x):
		...
		user_bias = self.user_bias(user_embed)
		item_bias = self.item_bias(item_embed)
		out = torch.sum(user_embed.mul(item_embed), -1, keepdim=True) / self.tau + user_bias + item_bias
...
args.expt_name = f"expt20_debug_expt19_{expt_num}"
...
for epoch in range(1, args.epochs+1):
	...
	if epoch % args.evaluate_interval == 0:
		...
		with torch.no_grad():
			...
			bias = model.user_bias(user_embed) + model.item_bias(item_embed).T
			user_score = torch.matmul(user_embed, item_embed.T) / model.tau + bias
