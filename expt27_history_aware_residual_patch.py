...
class JointRecStatic(nn.Module):
	def __init__(
		...
		self.hist_pad_idx = self.num_items
		...
		# user-specific temporal vector for item temporal context
		self.user_time_embedding = nn.Embedding(self.num_users, self.embedding_k)
		nn.init.zeros_(self.user_time_embedding.weight)

		# temporal feature encoder phi(v,t,H_t) -> c_v(t,H_t)
		self.temp_encoder = nn.Sequential(
			nn.Linear(self.temp_feat_dim, self.embedding_k),
			nn.LayerNorm(self.embedding_k),
			nn.GELU(),
			nn.Linear(self.embedding_k, self.embedding_k),
		)

		# history-aware user encoder h_u(t) from H_t^(u)
		self.hist_time_encoder = nn.Sequential(
			nn.Linear(2, self.embedding_k),
			nn.LayerNorm(self.embedding_k),
			nn.GELU(),
			nn.Linear(self.embedding_k, self.embedding_k),
		)
		self.history_ln = nn.LayerNorm(self.embedding_k)
		self.history_dropout = nn.Dropout(0.1)
	...
	def temporal_feature_vector(self, item_idx, query_time, item_time_all):
		"""
		item_idx: [B]
		query_time: [B]
		item_time_all: [B, max_pos]

		return:
			feat: [B, 4]
				1) normalized log intensity
				2) normalized recent count
				3) normalized long count
				4) normalized recent / long ratio
		"""
		item_embed = F.normalize(self.item_embedding(item_idx), dim=-1)   # [B, D]
		base = self.soft(self.base_fn(item_embed))                         # [B, 1]
		amplitude = self.soft(self.amplitude_fn(item_embed))              # [B, 1]

		mask = item_time_all < query_time.unsqueeze(1)                    # [B, max_pos]
		delta = (query_time.unsqueeze(1) - item_time_all).clamp(min=0.0) # [B, max_pos]
		intensity_decay = self.soft(self.intensity_decay)

		exp_term = torch.exp(-intensity_decay * delta) * mask             # [B, max_pos]
		time_intensity = exp_term.sum(dim=1, keepdim=True)                # [B, 1]
		log_intensity = torch.log(base + amplitude * time_intensity + 1e-12)

		# simple count features
		recent_mask = (delta <= 7.0) & mask       # last 7 days
		long_mask = (delta <= 30.0) & mask        # last 30 days

		recent_count = recent_mask.sum(dim=1, keepdim=True).float()
		long_count = long_mask.sum(dim=1, keepdim=True).float()
		ratio = recent_count / (long_count + 1e-6)

		feat = torch.cat([log_intensity, recent_count, long_count, ratio], dim=1)  # [B, 4]

		# batch normalization
		feat_mean = feat.mean(dim=0, keepdim=True)
		feat_std = feat.std(dim=0, keepdim=True) + 1e-6
		feat = (feat - feat_mean) / feat_std
		return feat

	def temporal_context(self, item_idx, query_time, item_time_all):
		feat = self.temporal_feature_vector(item_idx, query_time, item_time_all)  # [B, 4]
		ctx = self.temp_encoder(feat)                                              # [B, D]
		return ctx

	def encode_user_history(self, hist_item_idx, hist_time, query_time):
		"""
		hist_item_idx: [B, L] with padding id = self.num_items
		hist_time:     [B, L]
		query_time:    [B]
		return:
			hist_state: [B, D]
		"""
		if hist_item_idx is None:
			raise ValueError("hist_item_idx must be provided for history-aware residual learning.")

		if hist_item_idx.dim() == 1:
			hist_item_idx = hist_item_idx.unsqueeze(0)
		if hist_time.dim() == 1:
			hist_time = hist_time.unsqueeze(0)
		if query_time.dim() == 0:
			query_time = query_time.unsqueeze(0)

		valid_mask = (hist_item_idx != self.hist_pad_idx)                                    # [B, L]
		safe_hist_item_idx = hist_item_idx.clamp(min=0, max=self.num_items - 1)             # [B, L]
		hist_item_emb = self.item_resid_embedding(safe_hist_item_idx)                        # [B, L, D]
		hist_item_emb = hist_item_emb * valid_mask.unsqueeze(-1)

		delta = (query_time.unsqueeze(1) - hist_time).clamp(min=0.0)                         # [B, L]
		...
		time_emb = self.hist_time_encoder(time_feat)                                         # [B, L, D]
		time_emb = time_emb * valid_mask.unsqueeze(-1)

		seq_input = self.history_ln(hist_item_emb + time_emb)
		seq_input = self.history_dropout(seq_input)
		seq_input = seq_input * valid_mask.unsqueeze(-1)

		_, h_n = self.history_gru(seq_input)                                                 # [1, B, D]
		hist_state = h_n[-1]                                                                 # [B, D]

		empty_mask = (valid_mask.sum(dim=1, keepdim=True) == 0)
		hist_state = torch.where(empty_mask, torch.zeros_like(hist_state), hist_state)
		return hist_state

	def build_user_state(self, user_idx, hist_item_idx=None, hist_time=None, query_time=None):
		static_u = self.user_resid_embedding(user_idx)

		if hist_item_idx is not None:
			hist_state = self.encode_user_history(hist_item_idx, hist_time, query_time)
		else:
			hist_state = torch.zeros_like(static_u)

		user_pref = F.normalize(static_u + hist_state, dim=-1)                               # [B, D]
		user_time = F.normalize(self.user_time_embedding(user_idx) + hist_state, dim=-1)     # [B, D]
		return user_pref, user_time, hist_state
	...
	def residual_score(
		...
		u, g, hist_state = self.build_user_state(
			user_idx,
		...
		if item_ctx is not None:
			c = F.normalize(item_ctx, dim=-1)                                                # [B, D]
			score = score + torch.sum(g * c, dim=-1, keepdim=True)
	...
	def score_all_items(
		...
		base_score = torch.matmul(u, v_all.T) / self.tau

		if item_ctx_all is not None:
			c_all = F.normalize(item_ctx_all, dim=-1)                                        # [I, D]
			base_score = base_score + torch.matmul(g, c_all.T)                               # [B, I]
...
args.expt_name = f"expt27_history_aware_residual_patch_{expt_num}"
...
dataset.build_user_histories(max_seq_len=max_seq_len)
...
for epoch in range(1, args.epochs+1):
	...
	for idx in range(batch_num):
		...
		neg_user_np = dataset.neg_user_list[sample_idx, 0]
		neg_user = torch.tensor(neg_user_np, dtype=torch.long).to(args.device)

		anchor_item_np = dataset.item_list[sample_idx]
		anchor_item = torch.tensor(anchor_item_np, dtype=torch.long).to(args.device)

		query_time_np = dataset.event_time_list[sample_idx]
		query_time = torch.tensor(query_time_np, dtype=torch.float32).to(args.device)
		anchor_time_all = torch.tensor(dataset.item_time_array[anchor_item_np], dtype=torch.float32).to(args.device)

		pos_hist_item = torch.tensor(dataset.hist_item_list[sample_idx], dtype=torch.long).to(args.device)
		pos_hist_time = torch.tensor(dataset.hist_time_list[sample_idx], dtype=torch.float32).to(args.device)

		neg_hist_item_np, neg_hist_time_np = dataset.get_histories_for_users_at_times(
			neg_user_np,
			query_time_np,
			max_seq_len=max_seq_len,
		)
		neg_hist_item = torch.tensor(neg_hist_item_np, dtype=torch.long).to(args.device)
		neg_hist_time = torch.tensor(neg_hist_time_np, dtype=torch.float32).to(args.device)

		item_ctx = model.temporal_context(anchor_item, query_time, anchor_time_all)   # [B, D]

		pos_score, _, _ = model.residual_score(
			pos_user,
			anchor_item,
			item_ctx,
			hist_item_idx=pos_hist_item,
			hist_time=pos_hist_time,
			query_time=query_time,
		)
		neg_score, _, _ = model.residual_score(
			...
			anchor_item,
			item_ctx,
			hist_item_idx=neg_hist_item,
			hist_time=neg_hist_time,
			query_time=query_time,
		)
	...
	if epoch % args.evaluate_interval == 0:
		...
		for i, ((user, item), pos_time_val) in enumerate(dataset.valid_user_item_time.items()):
			...
			hist_item_np, hist_time_np = dataset.get_histories_for_users_at_times(
				np.array([user], dtype=np.int64),
				np.array([pos_time_val], dtype=np.float32),
				max_seq_len=max_seq_len,
			)
			hist_item = torch.tensor(hist_item_np, dtype=torch.long).to(args.device)
			hist_time = torch.tensor(hist_time_np, dtype=torch.float32).to(args.device)
			query_time_tensor = torch.tensor([pos_time_val], dtype=torch.float32).to(args.device)

			# build temporal feature vector for all items
			batch_time_all_full = torch.tensor(dataset.item_time_array, dtype=torch.float32).to(args.device)
			query_time_full = torch.full((dataset.m_item,), float(pos_time_val), dtype=torch.float32, device=args.device)
			item_idx_full = torch.arange(dataset.m_item, dtype=torch.long, device=args.device)

			with torch.no_grad():
				item_ctx_all = model.temporal_context(item_idx_full, query_time_full, batch_time_all_full)   # [I, D]
				residual_scores = model.score_all_items(
					user_idx,
					item_ctx_all,
					hist_item_idx=hist_item,
					hist_time=hist_time,
		...
		if wandb_login:
			wandb_var.log({"train_user_time_emb_norm": model.user_time_embedding.weight.norm(dim=1).mean().item()})
			wandb_var.log({
...
for i, ((user, item), pos_time_val) in enumerate(dataset.test_user_item_time.items()):
	...
	with torch.no_grad():
		item_ctx_all = best_model.temporal_context(item_idx_full, query_time_full, batch_time_all_full)   # [I, D]
		residual_scores = best_model.score_all_items(
			user_idx,
			item_ctx_all,
			hist_item_idx=hist_item,
			hist_time=hist_time,
