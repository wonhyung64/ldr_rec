...
class JointRecStatic(nn.Module):
	def __init__(
		...
		# new: user-specific temporal sensitivity alpha_u
		self.user_time_weight = nn.Embedding(self.num_users, 1)
		nn.init.zeros_(self.user_time_weight.weight)
	...
	def temporal_context(self, item_idx, query_time, item_time_all):
		...
		return: normalized scalar context c(v,t,H_t) with shape [B,1]
		"""
		...
		delta = (query_time.unsqueeze(1) - item_time_all).clamp(min=0.0)
		intensity_decay = self.soft(self.intensity_decay)
		time_intensity = (torch.exp(-intensity_decay * delta) * mask).sum(dim=1, keepdim=True)

		log_intensity = torch.log(base + amplitude * time_intensity + 1e-12)  # [B, 1]

		# batch-wise normalization to avoid scale blow-up
		mean = log_intensity.mean(dim=0, keepdim=True)
		std = log_intensity.std(dim=0, keepdim=True) + 1e-6
		ctx = (log_intensity - mean) / std
		return ctx
...
args.expt_name = f"expt24_simple_time_{expt_num}"
...
for epoch in range(1, args.epochs+1):
	...
	for idx in range(batch_num):
		...
		item_ctx = model.temporal_context(anchor_item, query_time, anchor_time_all)   # [B, 1]
	...
	if epoch % args.evaluate_interval == 0:
		...
		for i, ((user, item), pos_time_val) in enumerate(dataset.valid_user_item_time.items()):
			...
			with torch.no_grad():
				item_ctx_all = torch.log(item_logits + 1e-12)
				item_ctx_all = (item_ctx_all - item_ctx_all.mean()) / (item_ctx_all.std() + 1e-6)
				item_ctx_all = item_ctx_all.to(args.device)
