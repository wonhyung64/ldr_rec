...
class JointRecStatic(nn.Module):
	def __init__(
		...
		self.time_gap_proj = nn.Linear(1, self.embedding_k, bias=False)
	...
	def encode_user_history(self, hist_item_idx, hist_time, query_time):
		...
		"""
		if query_time.dim() == 1:
			query_time = query_time.unsqueeze(-1)   # [B, 1]

		hist_mask = (hist_item_idx != self.padding_item_id)   # [B, L]

		# item embedding
		hist_emb = self.item_resid_embedding(hist_item_idx)   # [B, L, D]

		# time-gap feature: log(1 + (t - t_j))
		time_gap = (query_time - hist_time).clamp(min=0.0)    # [B, L]
		time_gap = torch.log1p(time_gap) * hist_mask.float()
		time_gap_emb = self.time_gap_proj(time_gap.unsqueeze(-1))  # [B, L, D]

		# left padding 유지, pack 없이 full sequence를 그대로 통과
		gru_input = hist_emb + time_gap_emb * self.gamma
		_, h_n = self.user_gru(gru_input)
		u = h_n[-1]   # [B, D]
	...
	def residual_score(self, item_idx, hist_item_idx, hist_time, query_time):
		u = self.encode_user_history(hist_item_idx, hist_time, query_time)
		v = F.normalize(self.item_resid_embedding(item_idx), dim=-1)
		score = torch.sum(u * v, dim=-1, keepdim=True) / self.tau
		return score, u, v

	def score_all_items(self, hist_item_idx, hist_time, query_time):
		u = self.encode_user_history(hist_item_idx, hist_time, query_time)
		v_all = F.normalize(self.item_resid_embedding.weight[:self.num_items], dim=-1)
...
args.expt_name = f"expt30_debug_expt29{expt_num}"
...
for epoch in range(1, args.epochs+1):
	...
	epoch_pos_hist_len = 0.
	epoch_neg_hist_len = 0.
	epoch_pos_recent_gap = 0.
	epoch_neg_recent_gap = 0.
	...
	for idx in range(batch_num):
		...
		pos_hist_times = torch.tensor(
			dataset.hist_time_list[sample_idx],
			dtype=torch.float32
		).to(args.device)
		...
		neg_hist_times = torch.tensor(neg_hist_times_np, dtype=torch.float32).to(args.device)

		pos_score, _, _ = model.residual_score(
			anchor_item, pos_hist_items, pos_hist_times, query_time
		)
		...
		with torch.no_grad():
			...
			neg_hist_mask = (neg_hist_items != model.padding_item_id).float()   # [B, L]
			...
			# ---------- new debug: history length ----------
			pos_hist_len = pos_hist_mask.sum(dim=-1)   # [B]
			neg_hist_len = neg_hist_mask.sum(dim=-1)   # [B]

			batch_pos_hist_len = pos_hist_len.float().mean()
			batch_neg_hist_len = neg_hist_len.float().mean()

			# ---------- new debug: recent gap ----------
			# most recent valid interaction time = max valid hist_time
			pos_last_time = torch.where(
				pos_hist_mask.bool(),
				pos_hist_times,
				torch.full_like(pos_hist_times, -1e9),
			).max(dim=-1).values
			neg_last_time = torch.where(
				neg_hist_mask.bool(),
				neg_hist_times,
				torch.full_like(neg_hist_times, -1e9),
			).max(dim=-1).values

			pos_has_hist = (pos_hist_len > 0)
			neg_has_hist = (neg_hist_len > 0)

			if pos_has_hist.any():
				pos_recent_gap = (query_time - pos_last_time).clamp(min=0.0)
				batch_pos_recent_gap = pos_recent_gap[pos_has_hist].mean()
			else:
				batch_pos_recent_gap = torch.tensor(0.0, device=query_time.device)

			if neg_has_hist.any():
				neg_recent_gap = (query_time - neg_last_time).clamp(min=0.0)
				batch_neg_recent_gap = neg_recent_gap[neg_has_hist].mean()
			else:
				batch_neg_recent_gap = torch.tensor(0.0, device=query_time.device)
		...
		epoch_pos_hist_len += batch_pos_hist_len.item()
		epoch_neg_hist_len += batch_neg_hist_len.item()
		epoch_pos_recent_gap += batch_pos_recent_gap.item()
		epoch_neg_recent_gap += batch_neg_recent_gap.item()
	...
	if epoch % args.evaluate_interval == 0:
		...
		for i, ((user, item), pos_time_val) in enumerate(dataset.valid_user_item_time.items()):
			...
			hist_time_t = torch.tensor(hist_time_np, dtype=torch.float32).to(args.device)
			query_time_t = torch.tensor([pos_time_val], dtype=torch.float32).to(args.device)

			with torch.no_grad():
				residual_scores = model.score_all_items(
					hist_item_t, hist_time_t, query_time_t
				).squeeze(0)
		...
		if wandb_login:
			wandb_var.log({
				...
				# new debug
				"debug_pos_hist_len": epoch_pos_hist_len / batch_num,
				"debug_neg_hist_len": epoch_neg_hist_len / batch_num,
				"debug_pos_recent_gap": epoch_pos_recent_gap / batch_num,
				"debug_neg_recent_gap": epoch_neg_recent_gap / batch_num,
			})
		...
		current_valid_score = valid_results[2][0]
