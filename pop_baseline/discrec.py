...
def build_adj(train_dict, num_items=None):
    edges = [(a, b) for seq in train_dict.values() for a, b in zip(seq[:-1], seq[1:])]
    row, col = zip(*edges)

    n = num_items or max(max(row), max(col)) + 1
    data = np.ones(len(row), dtype=np.int8)

    # A_g(i,j)=1 if i 다음에 j가 인접
    return coo_matrix((data, (row, col)), shape=(n, n)).tocsr().sign()

A_g = build_adj(dataset.train_dict)
...
#%%
seqs = model.item_embedding(anchor_hist_items)
seqs *= model.item_embedding.embedding_dim ** 0.5
seqs = seqs + model.pos_enc(anchor_hist_items)
