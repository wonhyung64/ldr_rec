...
class JointRecStatic(nn.Module):
	...
	def residual_score(self, user_idx, item_idx, item_ctx=None):
		...
		if item_ctx is not None:
			g = F.normalize(self.user_time_embedding(user_idx), dim=-1)   # [B, D]
			c = F.normalize(item_ctx, dim=-1)                             # [B, D]
			score = score + torch.sum(g * c, dim=-1, keepdim=True)

		return score, u, v

	def score_all_items(self, user_idx, item_ctx_all=None):
		u = F.normalize(self.user_resid_embedding(user_idx), dim=-1)      # [B, D]
...
args.expt_name = f"expt25_user_time_interact_{expt_num}"
...
for epoch in range(1, args.epochs+1):
	...
	for idx in range(batch_num):
		...
		query_time = torch.tensor(dataset.event_time_list[sample_idx], dtype=torch.float32).to(args.device)
		...
		pos_score, _, _ = model.residual_score(pos_user, anchor_item, item_ctx)
		neg_score, _, _ = model.residual_score(neg_user, anchor_item, item_ctx)
	...
	if epoch % args.evaluate_interval == 0:
		...
		for i, ((user, item), pos_time_val) in enumerate(dataset.valid_user_item_time.items()):
			...
			with torch.no_grad():
				...
				residual_scores = model.score_all_items(user_idx, item_ctx_all).squeeze(0)                    # [I]
...
for i, ((user, item), pos_time_val) in enumerate(dataset.test_user_item_time.items()):
	...
	with torch.no_grad():
		...
		residual_scores = best_model.score_all_items(user_idx, item_ctx_all).squeeze(0)
